...
// 	$rtf->setOperator("me at noginn.com");
...
class rtf {
	...
	function getDocument() {
        ...
        if (defined('URL_PARAM_PAGE_NAME')) {
            $pageName = $_REQUEST[URL_PARAM_PAGE_NAME];
            if (is_numeric(strpos($pageName, '|'))) {
                $pageName = substr($pageName, 0, strpos($pageName, '|'));
                $pageName = trim(preg_replace('~ \d+~', '', $pageName));          
            }
            $pageName = str_replace(array('.', ' '), '_', $pageName);
            $pageName = str_replace('__', '_', $pageName);
            $pageName = str_replace('Psalm', 'Psalmen', $pageName);            
            #var_dump($pageName);
    		header("Content-Disposition: attachment; filename=$pageName.doc");
        } else {
	...
	function parseDocument() {
        ...
        $doc_buffer = preg_replace("/<H4.*>(.*?)<\/H4>/mi", "\\fs22\\b \\1\\b0\\fs{$this->font_size}\\par ", $doc_buffer);


		$doc_buffer = preg_replace("/<HR(.*?)>/i", "\\brdrb\\brdrs\\brdrw30\\brsp20 \\pard\\par ", $doc_buffer);
		$doc_buffer = str_replace(array("<BR>", '<br />'), "\\par ", $doc_buffer);


        $doc_buffer = str_replace("<TAB>", "\\tab ", $doc_buffer);

        $doc_buffer = str_replace(array('<ol>', '<ol />'), '', $doc_buffer);
        
        $doc_buffer = str_replace(RTF_PAGE_BREAK_PATTERN,   '\page ',   $doc_buffer);
        $doc_buffer = str_replace('&hellip;',               ' \\\'85 ', $doc_buffer);
        #$doc_buffer = str_replace('&rsquo;',                ' \\\'93 ', $doc_buffer); // @todo            
            
       
        

		$doc_buffer = $this->nl2par($doc_buffer);
        $doc_buffer = str_replace(array('<p>', '</p>'), '', $doc_buffer);        
